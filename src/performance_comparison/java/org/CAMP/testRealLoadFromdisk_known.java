 ...
 * 
 */
public class testRealLoadFromdisk_known {
   ...
   //     static final String[] ops = { AND, OR, XOR };
	...
	public static int [][] id_list;
        ...
        public static void testDataset(String file_format, String[] seq, Boolean IPorPort, int max, int [] list) throws IOException{
...
//                			System.out.println(i + Arrays.toString(a));
	    ...
	    id_list = new int[4][100];
	    FileReader fr = new FileReader("src_ip_id_list"); 
            BufferedReader br = new BufferedReader(fr); 
	    String s;
            String [] file_name_list = {"src_ip_id_list", "dst_ip_id_list", "src_port_id_list", "dst_port_id_list"};
	    for(int j = 0; j<file_name_list.length; j++)
	    {
		fr = new FileReader(file_name_list[j]);
		br = new BufferedReader(fr);
		s = br.readLine();
		int index = 0;
		while(s!= null)
	  	{
		   id_list[j][index++] = Integer.valueOf(s);
		   s= br.readLine();     
		}
	    }
	    String format = "src_ip";
            ...
            testDataset(format, get_known_sequence(file_name), IPorPort, max_value,id_list[0]);
            ...
            testDataset(format, get_known_sequence(file_name), IPorPort, max_value,id_list[1]);
            format = "src_port";
        	file_name = format + "_list.txt";
            IPorPort = false;
            
            testDataset(format, get_known_sequence(file_name), IPorPort, max_value,id_list[2]);
            format = "dst_port";
        	file_name = format + "_list.txt";
            IPorPort = false;
            
            testDataset(format, get_known_sequence(file_name), IPorPort, max_value,id_list[3]);
