...
public class operation_set {
	...
	static int num = (int)Math.ceil((double)create_index_set.block_size/((int)(Math.sqrt(create_index_set.block_size) * 0.1)));
	public static index_set intersection(index_set id1, index_set id2)
	{
		index_set result = new index_set();
		
		
		int size = id1.size > id2.size ? id2.size : id1.size;
		result.id = new index1[size];
		result.size = size;
		for(int i = 0; i<size;i++)
		{
			result.id[i] =(intersection(id1.id[i], id2.id[i]));
		}
		num = result.num;
		return result;
	}
	...
	public static index_set union(index_set id1, index_set id2)
	{
		index_set result = new index_set();
		
		int size = id1.size > id2.size ? id2.size : id1.size;
		int big_size = id1.size < id2.size ? id2.size : id1.size;
		result.id = new index1[big_size];
		result.size = big_size;
		for(int i = 0; i<size;i++)
		{
			result.id[i] = (union(id1.id[i], id2.id[i]));
		}
		
		if(id1.size > id2.size)
		for(int i = size;i<id1.size;i++)
		{
			
			result.id[i] = id1.id[i];
		}
		
		if(id1.size < id2.size)
			for(int i = size;i<id2.size;i++)
			{
				
				result.id[i] = id2.id[i];
			}
	...
	public static index1 intersection(index1 id1, index1 id2)
	{
...
//							prefix_tp.clear();
	...
	public static index1 union(index1 id1, index1 id2)
	{
