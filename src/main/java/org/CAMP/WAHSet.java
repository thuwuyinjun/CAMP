...
public class WAHSet
{
  ...
  public static WAHSet newImmutableFromMutable(ConciseSet conciseSet)
  {
    if (conciseSet == null || conciseSet.isEmpty()) {
      return new WAHSet();
    }
    return new WAHSet(IntBuffer.wrap(conciseSet.getWords()));
  }
  ...
  public static WAHSet union(WAHSet... sets)
  {
    return union(Arrays.asList(sets));
  }

  public static WAHSet union(Iterable<WAHSet> sets)
  {
    return union(sets.iterator());
  }

  public static WAHSet union(Iterator<WAHSet> sets)
  {
    WAHSet partialResults = doUnion(Iterators.limit(sets, CHUNK_SIZE));
    while (sets.hasNext()) {
      final UnmodifiableIterator<WAHSet> partialIter = Iterators.singletonIterator(partialResults);
      partialResults = doUnion(Iterators.<WAHSet>concat(partialIter, Iterators.limit(sets, CHUNK_SIZE)));
    }
    return partialResults;
  }

  public static WAHSet intersection(WAHSet... sets)
  {
    return intersection(Arrays.asList(sets));
  }

  public static WAHSet intersection(Iterable<WAHSet> sets)
  {
    return intersection(sets.iterator());
  }

  public static WAHSet intersection(Iterator<WAHSet> sets)
  {
    WAHSet partialResults = doIntersection(Iterators.limit(sets, CHUNK_SIZE));
    while (sets.hasNext()) {
      final UnmodifiableIterator<WAHSet> partialIter = Iterators.singletonIterator(partialResults);
      partialResults = doIntersection(
          Iterators.<WAHSet>concat(Iterators.limit(sets, CHUNK_SIZE), partialIter)
      );
    }
    return partialResults;
  }

  public static WAHSet complement(WAHSet set)
  {
    return doComplement(set);
  }

  public static WAHSet complement(WAHSet set, int length)
  {
    if (length <= 0) {
      return new WAHSet();
    }
    ...
    if (set == null || set.isEmpty()) {
      ...
      return WAHSet.newImmutableFromMutable(newSet);
    }
    ...
    return compact(new WAHSet(IntBuffer.wrap(retVal.toArray())));
  }

  public static WAHSet bitmapof(int []data)
  {
	  ...
	  return WAHSet.newImmutableFromMutable(concise);
  }
  
  public static WAHSet compact(WAHSet set)
  {
    IntList retVal = new IntList();
    ...
    return new WAHSet(IntBuffer.wrap(retVal.toArray()));
  }
  ...
  private static WAHSet doUnion(Iterator<WAHSet> sets)
  {
    IntList retVal = new IntList();
  ...
  public static WAHSet doIntersection(Iterator<WAHSet> sets)
  {
    IntList retVal = new IntList();
  ...
  public static WAHSet doComplement(WAHSet set)
  {
    if (set == null || set.isEmpty()) {
      return new WAHSet();
    }
  ...
  public WAHSet(ByteBuffer byteBuffer)
  {
  ...
  public WAHSet(IntBuffer buffer)
  {
    this.words = buffer;
  ...
  public int compareTo(WAHSet other)
  {
